...
export default function SocketGameBoard({ uuid, perspective, _gameBoardComponentType, setGameEndedResult, returnGameAfterLoad, externalMoveNotify }) {
    ...
    useEffect(() => {
        const socket = io(window.location.host, { query: { gameUUID: uuid }, withCredentials: true });
        ...
        function sendMove(move) {
            const event = move.resigned == undefined ? 'move' : 'resign';
            socket.emit(event, JSON.stringify(move));
        }
